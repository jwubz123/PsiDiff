from models.energys_prediction import utils
import numpy as np
import math
import torch
from models.g_phi_net import models
...
def sigmoid_beta_schedule(beta_start=1.e-7, beta_end=2.e-3, num_diffusion_timesteps=4999):
...
class PredefinedNoiseSchedule(torch.nn.Module):
    ...
    def __init__(self, noise_schedule, timesteps, precision):
        ...
        if noise_schedule == 'cosine':
            ...
            alphas2 = sigmoid_beta_schedule(beta_start=1.e-7, beta_end=2.e-3, num_diffusion_timesteps=timesteps)
        else:
...
class EnergyDiffusion(torch.nn.Module):
    ...
    def __init__(
            self,
            dynamics: models.EGNN_energy, in_node_nf: int, n_dims: int,
    ...
    def normalize(self, x, h, node_mask):
        ...
        h_cat = (h.float() - self.norm_biases[1]) / self.norm_values[1] * node_mask
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, out_mask, context, label):
        ...
        z_xh_t = torch.cat([z_t, h], dim=2)


        prediction = self.phi(z_xh_t, t, node_mask, edge_mask, context)
        
        if len(label.shape) > 1:
            prediction = prediction[node_mask.squeeze(-1)]
            label = label[node_mask.squeeze(-1)]
        loss = loss_l1(prediction, label)

        return loss

    def forward(self, x, h, node_mask=None, edge_mask=None, out_mask=None, context=None, label=None):
        """
        Computes the loss (type l2 or NLL) if training. And if eval then always computes NLL.
        """
        # Normalize data, take into account volume change in x.
        x, h = self.normalize(x, h, node_mask)
        loss = self.compute_loss(x, h, node_mask, edge_mask, out_mask, context, label)
        return loss
