...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
                 ...
                 sin_embedding=False, normalization_factor=100, aggregation_method='sum', out_dim=1):
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None,n_nodes=None):
        ...
        h = self.node_dec(h)
        h = h * node_mask # (B*N, H)
        h = h.view(-1, n_nodes, self.hidden_nf) # (B, N, H)
        if self.out_dim == 1:
            h = torch.sum(h, dim=1) # (B, 1, H)
        pred = self.graph_dec(h) # (B, 1)
        return pred.squeeze(-1) # torch.Size([B])
