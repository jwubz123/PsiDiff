...
from models.g_phi_net.egnn import EGNN
import numpy as np

class EGNN_energy(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
                 n_dims, hidden_nf=64, device='cpu',
                 act_fn=torch.nn.SiLU(), n_layers=4, attention=False,
                 condition_time=True, tanh=False, mode='egnn_dynamics', norm_constant=0,
                 inv_sublayers=2, sin_embedding=False, normalization_factor=100, aggregation_method='sum', out_dim=1):
        super().__init__()
        self.mode = mode
    ...
    def _forward(self, t, xh, node_mask, edge_mask, context):
        ...
        if h_dims == 0:
            ...
            h = xh[:, self.n_dims:].clone() # (B*n_nodes, 28)
        ...
        if self.condition_time:
            ...
            h = torch.cat([h, h_time], dim=1) # (B*n_nodes, 29)
